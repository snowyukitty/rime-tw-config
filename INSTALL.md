...
> 2026-06-25 盤點：**Rime 早已安裝並設定妥當**，毋須重裝、毋須重啟。

## 實機現況
- 已安裝 **Weasel 0.17.0**（穩定的 0.17 線，非會崩的 0.16.x）。
- 方案清單：`luna_pinyin_tw`（臺灣正體，**預設**）、`luna_pinyin_simp`（簡體）。
...
2. 若候選出的是簡體（为），按 `F4` 叫出方案選單 → 選
   **「朙月拼音·臺灣正體」**。選一次後會記住。
3. 測試：`wei`→為、`zhong`→眾、`li`→裡、`me`→麼、`qun`→群。
4. 外觀若沒變，右鍵小狼毫圖示 →「重新部署」。

## 防止「爲」復發（建議）
- 設定 → 時間與語言 → 語言與地區 → 中文(繁體) → 語言選項 →
  **移除「微軟注音/微軟拼音」**，只留小狼毫，就不會誤切回去打出「爲」。

## 可選：升級 0.17.0 → 0.17.4（與解決問題無關）
- 純小升級，**不影響字形**。`winget upgrade Rime.Weasel` 即可。
...
- 解法（已驗證）：以系統管理員執行官方工具補註冊（`/t` = 繁體）：
  `& 'C:\Program Files\Rime\weasel-0.17.0\WeaselSetup.exe' /t`
